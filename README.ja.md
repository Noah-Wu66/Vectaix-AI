...
Vectaix AI は、Vercel クラウドネイティブデプロイ向けに設計された**オープンソース AI ワークスペース**です。世界をリードする AI モデルを統一インターフェースに集約し、公式 API サポート、インテリジェントルーティング、独自のマルチエキスパート協調システムを提供します。
...
### 🧠 Council ワークフロー
独自のマルチエキスパート協調モード：GPT-5.4、Claude Opus 4.6、Gemini 2.5 Pro が並列エキスパートとして回答し、Seed 2.0 Pro が最終回答を統合。

### 🔀 スマートモデルルーティング
ユーザーレベルの公式 API と OpenRouter 間のルート切り替えで、柔軟性とコスト管理を実現。
...
| モデル | プロバイダー | ルーティング | 特徴 |
|:-------|:-------------|:-------------|:-----|
| **GPT-5.4** | OpenAI | 公式 / OpenRouter | 汎用知能、コーディング、分析 |
| **Claude Opus 4.6** | Anthropic | 公式 / OpenRouter | 深い推論、ライティング、安全性 |
| **Gemini 2.5 Pro** | Google | 公式 / OpenRouter | マルチモーダル、長文コンテキスト |
| **DeepSeek V3.2** | DeepSeek | 公式のみ | 推論、数学、コード |
| **Seed 2.0 Pro** | ByteDance | 公式のみ | 中国語、要約 |
| **MiMo V2 Flash** | Xiaomi | OpenRouter のみ | 高速推論 |
| **MiniMax M2.5** | MiniMax | OpenRouter のみ | 多言語生成 |
...
| AI SDK | @anthropic-ai/sdk、@google/genai、OpenAI REST、Volcengine Seed |
