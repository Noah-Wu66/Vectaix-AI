...
const ALLOWED_TIMELINE_KINDS = new Set(["thought", "search", "reader", "sandbox", "tool", "approval", "upload", "parse"]);
...
const MAX_AGENT_STATUS_CHARS = 64;
const MAX_AGENT_STEP_CHARS = 200;
const MAX_AGENT_ERROR_CHARS = 2000;
const MAX_AGENT_STEPS = 20;
const MAX_AGENT_ARTIFACTS = 10;
const MAX_CHAT_RUN_PHASE_CHARS = 64;
...
export function sanitizeMessage(message, index, { allowContentFallback = false } = {}) {
  ...
  if (isPlainObject(message.agentRun)) {
    const agentRun = {};
    if (typeof message.agentRun.runId === "string" && message.agentRun.runId.length <= MAX_MESSAGE_ID_CHARS) {
      agentRun.runId = message.agentRun.runId;
    }
    if (typeof message.agentRun.status === "string" && message.agentRun.status.length <= MAX_AGENT_STATUS_CHARS) {
      agentRun.status = message.agentRun.status;
    }
    if (typeof message.agentRun.currentStep === "string") {
      agentRun.currentStep = message.agentRun.currentStep.slice(0, MAX_AGENT_STEP_CHARS);
    }
    if (typeof message.agentRun.executionState === "string" && message.agentRun.executionState.length <= MAX_AGENT_STATUS_CHARS) {
      agentRun.executionState = message.agentRun.executionState;
    }
    if (Number.isFinite(message.agentRun.currentCursor)) {
      agentRun.currentCursor = message.agentRun.currentCursor;
    }
    if (Number.isFinite(message.agentRun.attemptCount)) {
      agentRun.attemptCount = message.agentRun.attemptCount;
    }
    if (typeof message.agentRun.lastError === "string") {
      agentRun.lastError = message.agentRun.lastError.slice(0, MAX_AGENT_ERROR_CHARS);
    }
    if (typeof message.agentRun.failureReason === "string") {
      agentRun.failureReason = message.agentRun.failureReason.slice(0, MAX_AGENT_ERROR_CHARS);
    }
    if (typeof message.agentRun.approvalReason === "string") {
      agentRun.approvalReason = message.agentRun.approvalReason.slice(0, MAX_MESSAGE_CHARS);
    }
    if (typeof message.agentRun.approvalStatus === "string" && message.agentRun.approvalStatus.length <= MAX_AGENT_STATUS_CHARS) {
      agentRun.approvalStatus = message.agentRun.approvalStatus;
    }
    if (typeof message.agentRun.canResume === "boolean") {
      agentRun.canResume = message.agentRun.canResume;
    }
    if (Array.isArray(message.agentRun.steps)) {
      agentRun.steps = message.agentRun.steps
        .slice(0, MAX_AGENT_STEPS)
        .map((step) => {
          if (!isPlainObject(step)) return null;
          const next = {};
          if (typeof step.type === "string") next.type = step.type.slice(0, MAX_AGENT_STATUS_CHARS);
          if (typeof step.title === "string") next.title = step.title.slice(0, MAX_AGENT_STEP_CHARS);
          if (typeof step.status === "string") next.status = step.status.slice(0, MAX_AGENT_STATUS_CHARS);
          if (typeof step.toolName === "string") next.toolName = step.toolName.slice(0, MAX_AGENT_STATUS_CHARS);
          return Object.keys(next).length > 0 ? next : null;
        })
        .filter(Boolean);
    }
    if (Array.isArray(message.agentRun.artifacts)) {
      agentRun.artifacts = message.agentRun.artifacts
        .slice(0, MAX_AGENT_ARTIFACTS)
        .map((artifact) => {
          ...
          const next = { url };
          if (typeof artifact.title === "string") next.title = artifact.title.slice(0, MAX_TITLE_CHARS);
          if (typeof artifact.mimeType === "string") next.mimeType = artifact.mimeType.slice(0, 128);
          if (typeof artifact.extension === "string") next.extension = artifact.extension.slice(0, 32);
          if (Number.isFinite(artifact.size)) next.size = artifact.size;
          return next;
        })
        .filter(Boolean);
    }
    if (Array.isArray(message.agentRun.citations)) {
      agentRun.citations = sanitizeCitations(message.agentRun.citations, `messages[${index}].agentRun.citations`);
    }
    if (isPlainObject(message.agentRun.sandboxSession)) {
      const sandboxSession = {};
      if (typeof message.agentRun.sandboxSession.provider === "string") {
        sandboxSession.provider = message.agentRun.sandboxSession.provider.slice(0, MAX_AGENT_STATUS_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.runtime === "string") {
        sandboxSession.runtime = message.agentRun.sandboxSession.runtime.slice(0, MAX_AGENT_STATUS_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.sandboxId === "string") {
        sandboxSession.sandboxId = message.agentRun.sandboxSession.sandboxId.slice(0, MAX_MESSAGE_ID_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.snapshotId === "string") {
        sandboxSession.snapshotId = message.agentRun.sandboxSession.snapshotId.slice(0, MAX_MESSAGE_ID_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.status === "string") {
        sandboxSession.status = message.agentRun.sandboxSession.status.slice(0, MAX_AGENT_STATUS_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.workdir === "string") {
        sandboxSession.workdir = message.agentRun.sandboxSession.workdir.slice(0, MAX_URL_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.lastConnectedAt === "string") {
        sandboxSession.lastConnectedAt = message.agentRun.sandboxSession.lastConnectedAt.slice(0, 64);
      }
      if (typeof message.agentRun.sandboxSession.requiresApproval === "boolean") {
        sandboxSession.requiresApproval = message.agentRun.sandboxSession.requiresApproval;
      }
      if (typeof message.agentRun.sandboxSession.canResume === "boolean") {
        sandboxSession.canResume = message.agentRun.sandboxSession.canResume;
      }
      if (isPlainObject(message.agentRun.sandboxSession.latestCommand)) {
        const latestCommand = {};
        if (typeof message.agentRun.sandboxSession.latestCommand.cmdId === "string") {
          latestCommand.cmdId = message.agentRun.sandboxSession.latestCommand.cmdId.slice(0, MAX_MESSAGE_ID_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.latestCommand.command === "string") {
          latestCommand.command = message.agentRun.sandboxSession.latestCommand.command.slice(0, MAX_MESSAGE_CHARS);
        }
        if (Number.isFinite(message.agentRun.sandboxSession.latestCommand.exitCode)) {
          latestCommand.exitCode = message.agentRun.sandboxSession.latestCommand.exitCode;
        }
        if (typeof message.agentRun.sandboxSession.latestCommand.stdout === "string") {
          latestCommand.stdout = message.agentRun.sandboxSession.latestCommand.stdout.slice(0, MAX_MESSAGE_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.latestCommand.stderr === "string") {
          latestCommand.stderr = message.agentRun.sandboxSession.latestCommand.stderr.slice(0, MAX_MESSAGE_CHARS);
        }
        if (Object.keys(latestCommand).length > 0) {
          sandboxSession.latestCommand = latestCommand;
        }
      }
      if (isPlainObject(message.agentRun.sandboxSession.pendingCommand)) {
        const pendingCommand = {};
        if (typeof message.agentRun.sandboxSession.pendingCommand.cmdId === "string") {
          pendingCommand.cmdId = message.agentRun.sandboxSession.pendingCommand.cmdId.slice(0, MAX_MESSAGE_ID_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.command === "string") {
          pendingCommand.command = message.agentRun.sandboxSession.pendingCommand.command.slice(0, MAX_MESSAGE_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.status === "string") {
          pendingCommand.status = message.agentRun.sandboxSession.pendingCommand.status.slice(0, MAX_AGENT_STATUS_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.startedAt === "string") {
          pendingCommand.startedAt = message.agentRun.sandboxSession.pendingCommand.startedAt.slice(0, 64);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.stdoutPath === "string") {
          pendingCommand.stdoutPath = message.agentRun.sandboxSession.pendingCommand.stdoutPath.slice(0, MAX_URL_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.stderrPath === "string") {
          pendingCommand.stderrPath = message.agentRun.sandboxSession.pendingCommand.stderrPath.slice(0, MAX_URL_CHARS);
        }
        if (typeof message.agentRun.sandboxSession.pendingCommand.exitCodePath === "string") {
          pendingCommand.exitCodePath = message.agentRun.sandboxSession.pendingCommand.exitCodePath.slice(0, MAX_URL_CHARS);
        }
        if (Object.keys(pendingCommand).length > 0) {
          sandboxSession.pendingCommand = pendingCommand;
        }
      }
      if (Object.keys(sandboxSession).length > 0) {
        agentRun.sandboxSession = sandboxSession;
      }
    }
    if (typeof message.agentRun.updatedAt === "string" && message.agentRun.updatedAt.length <= 64) {
      agentRun.updatedAt = message.agentRun.updatedAt;
    }
    if (Object.keys(agentRun).length > 0) {
      out.agentRun = agentRun;
    }
  }

  if (isPlainObject(message.chatRun)) {
    const chatRun = {};
    if (typeof message.chatRun.runId === "string" && message.chatRun.runId.length <= MAX_MESSAGE_ID_CHARS) {
      chatRun.runId = message.chatRun.runId;
    }
    if (typeof message.chatRun.status === "string" && message.chatRun.status.length <= MAX_AGENT_STATUS_CHARS) {
      chatRun.status = message.chatRun.status;
    }
    if (typeof message.chatRun.phase === "string" && message.chatRun.phase.length <= MAX_CHAT_RUN_PHASE_CHARS) {
      chatRun.phase = message.chatRun.phase;
    }
    if (typeof message.chatRun.provider === "string" && message.chatRun.provider.length <= MAX_AGENT_STATUS_CHARS) {
      chatRun.provider = message.chatRun.provider;
    }
    if (typeof message.chatRun.model === "string" && message.chatRun.model.length <= MAX_MODEL_CHARS) {
      chatRun.model = message.chatRun.model;
    }
    if (typeof message.chatRun.errorMessage === "string") {
      chatRun.errorMessage = message.chatRun.errorMessage.slice(0, MAX_AGENT_ERROR_CHARS);
    }
    if (typeof message.chatRun.updatedAt === "string" && message.chatRun.updatedAt.length <= 64) {
      chatRun.updatedAt = message.chatRun.updatedAt;
    }
    if (Object.keys(chatRun).length > 0) {
      out.chatRun = chatRun;
    }
  }
