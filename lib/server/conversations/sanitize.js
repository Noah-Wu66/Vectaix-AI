...
export function sanitizeMessage(message, index, { allowContentFallback = false } = {}) {
  ...
  if (isPlainObject(message.agentRun)) {
    ...
    if (isPlainObject(message.agentRun.sandboxSession)) {
      ...
      if (typeof message.agentRun.sandboxSession.template === "string") {
        sandboxSession.template = message.agentRun.sandboxSession.template.slice(0, MAX_TITLE_CHARS);
      }
      if (typeof message.agentRun.sandboxSession.templateVersion === "string") {
        sandboxSession.templateVersion = message.agentRun.sandboxSession.templateVersion.slice(0, MAX_AGENT_STATUS_CHARS);
      }
