...
export async function prepareDocumentAttachmentMapByUrls(urls, {
  userId,
  conversationId = null,
  signal,
} = {}) {
  ...
  const uniqueUrls = Array.from(new Set(
    Array.isArray(urls) ? urls.filter((item) => typeof item === "string" && item) : []
  ));

  for (const url of uniqueUrls) {
    const result = await prepareDocumentAttachment({
      userId,
      url,
      conversationId,
      signal,
