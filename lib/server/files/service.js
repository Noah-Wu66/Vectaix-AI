...
import {
  ...
  getE2BTemplateVersion,
  parseAttachmentInSandbox,
} from "@/lib/server/sandbox/e2b";
...
function assertPreparedWithinLimits(prepared, category, limits = getAttachmentLimits(category)) {
  if (!limits) return;
  if (category === "document" && limits.maxPages && Number(prepared?.pageCount) > limits.maxPages) {
    throw new Error(`PDF 页数超过限制，最多支持 ${limits.maxPages} 页`);
  }
  if (category !== "spreadsheet") return;
  if (limits.maxSheets && Number(prepared?.sheetCount) > limits.maxSheets) {
    throw new Error(`工作表数量超过限制，最多支持 ${limits.maxSheets} 个`);
  }
  if (limits.maxRowsPerSheet && Number(prepared?.rowCount) > limits.maxRowsPerSheet * Math.max(Number(prepared?.sheetCount) || 1, 1)) {
    throw new Error(`表格总行数超过限制，最多支持约 ${limits.maxRowsPerSheet} 行/表`);
  }
  if (limits.maxCols && Number(prepared?.maxCols) > limits.maxCols) {
    throw new Error(`表格列数超过限制，最多支持 ${limits.maxCols} 列`);
  }
  if (limits.maxCells && Number(prepared?.cellCount) > limits.maxCells) {
...
export async function prepareDocumentAttachment({
  ...
  if (isReadyBlobFile(blobFile)) {
    ...
    if (!conversationId && !sandboxSession?.sandboxId) {
      return {
        prepared,
        sandboxSession,
        commandResult: null,
  ...
  try {
    ...
    await BlobFile.updateOne(
      { _id: blobFile._id },
      {
        $set: {
          ...
          sandboxPath: parsed.sandboxPath || null,
    ...
    return {
      ...
      sandboxSession: parsed.session || sandboxSession,
