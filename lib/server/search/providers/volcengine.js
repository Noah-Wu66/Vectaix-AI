...
export const VOLCENGINE_SEARCH_TYPES = Object.freeze({
  image: "image",
  web: "web",
  webSummary: "web_summary",
});

const VOLCENGINE_ERROR_LABELS = Object.freeze({
  "10400": "请求参数不合法",
  "10402": "没有可用结果",
  "10403": "鉴权失败",
  "10500": "火山搜索服务内部错误",
...
function normalizeVolcengineResult(item) {
  ...
  return {
    title: normalizeTextValue(item.Title) || url,
    url,
    snippet: normalizeTextValue(item.Snippet ?? item.Summary ?? item.Content),
    siteName: normalizeTextValue(item.SiteName) || extractHostname(url),
    datePublished: normalizeTextValue(item.PublishTime),
    lastUpdated: "",
  };
}
...
function normalizeSearchType(searchType) {
  if (searchType === VOLCENGINE_SEARCH_TYPES.image) return VOLCENGINE_SEARCH_TYPES.image;
  if (searchType === VOLCENGINE_SEARCH_TYPES.webSummary) return VOLCENGINE_SEARCH_TYPES.webSummary;
  return VOLCENGINE_SEARCH_TYPES.web;
}

export function buildVolcengineSearchRequestBody(query, options = {}) {
  ...
  const body = {
    ...
    Count: maxResults,
    Filter: {
      NeedContent: options.needContent !== false,
      NeedUrl: options.needUrl !== false,
      AuthInfoLevel: Number(options.authInfoLevel) === 1 ? 1 : 0,
    },
  };
  ...
  if (searchType === VOLCENGINE_SEARCH_TYPES.webSummary) {
    body.NeedSummary = true;
  }
  if (timeRange) body.TimeRange = timeRange;
  if (sites) body.Filter.Sites = sites;
  if (blockHosts) body.Filter.BlockHosts = blockHosts;
...
function buildRequestSignal({ timeoutMs = VOLCENGINE_TIMEOUT_MS, signal } = {}) {
  ...
  if (signal && hasFiniteTimeout) return AbortSignal.any([signal, AbortSignal.timeout(timeoutMs)]);
...
function getVolcengineErrorMeta(payload) {
  ...
  return {
    code,
    message,
    displayMessage: VOLCENGINE_ERROR_LABELS[code]
      ? `${VOLCENGINE_ERROR_LABELS[code]}${message ? `：${message}` : ""}`
      : (message || code),
  };
}

export function summarizeVolcengineResponse(rawText) {
  ...
  for (const payload of payloads) {
    const errorMeta = getVolcengineErrorMeta(payload);
    if (errorMeta) {
      const error = new Error(errorMeta.displayMessage || "Volcengine API error");
      error.code = errorMeta.code || "";
      error.meta = errorMeta;
      throw error;
    }

    requestId ||= normalizeTextValue(payload?.ResponseMetadata?.RequestId ?? payload?.Result?.LogId);

    const webResults = Array.isArray(payload?.Result?.WebResults) ? payload.Result.WebResults : [];
    if (webResults.length > 0) rawResults.push(...webResults);
    ...
    for (const choice of choices) {
      ...
      if (deltaText) summaryFromDelta += deltaText;
  ...
  return enqueueVolcengineSearch(async () => {
    ...
    for (let attempt = 0; attempt < VOLCENGINE_MAX_RETRIES; attempt += 1) {
      ...
      try {
        ...
        lastError = error;
        if (signal?.aborted) throw toAbortError(signal);
        if (attempt >= VOLCENGINE_MAX_RETRIES - 1) throw error;
    ...
    if (!response?.ok) {
      ...
      try {
        ...
        const errorMeta = getVolcengineErrorMeta(payload);
        message = errorMeta?.displayMessage || message;
      } catch {}
...
async function volcengineSearch(query, options = {}) {
...
export async function volcengineWebSearch(query, options = {}) {
  return volcengineSearch(query, {
    ...options,
    searchType: VOLCENGINE_SEARCH_TYPES.webSummary,
  });
}

export async function volcengineWebSearchResults(query, options = {}) {
  return volcengineSearch(query, {
    ...options,
    searchType: VOLCENGINE_SEARCH_TYPES.web,
  });
}
