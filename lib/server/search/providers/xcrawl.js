...
function resolveSearchLocale(query) {
  ...
  const location = language === "zh" ? "" : (language === "ja" ? "JP" : "US");
  return { location, language };
}
...
export async function xcrawlSearch(query, options = {}) {
  ...
  const payload = await requestXCrawl("/v1/search", requestBody, {
  ...
  const upstreamStatus = typeof payload?.data?.status === "string" ? payload.data.status.trim().toLowerCase() : "";

  if (payloadStatus === "failed" || upstreamStatus === "failed") {
    console.error("[XCrawl] Search failed", {
      query: clipLogValue(normalizedQuery),
      resolved,
      payload: summarizeXCrawlPayload("/v1/search", payload),
    });
    throw new Error("XCrawl search failed");
  }
