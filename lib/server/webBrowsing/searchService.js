import { tavilyExtract, tavilySearch } from "@/lib/server/search/providers/tavily";
import {
  ...
  async webSearch({ query }, options = {}) {
    try {
      ...
      console.error("[WebSearch] Search failed", {
        errorType: error instanceof Error ? error.name : "SearchError",
      });
      return {
        costTime: 0,
    ...
    const results = await mapWithConcurrency(
      ...
      async (url) => {
        try {
          ...
          const name = error instanceof Error ? error.name : "TavilyExtractError";
