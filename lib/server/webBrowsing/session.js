  ...
  return {
    urls: Array.isArray(args.urls)
      ? args.urls
        ...
        .map((item) => item.trim())
        .slice(0, WEB_BROWSING_MAX_CRAWL_URLS)
      : [],
  };
}
...
function buildToolLoopPrompt({ prompt, historyMessages, toolCalls, availableToolApiNames, roundState }) {
  const safeAvailableToolApiNames = Array.isArray(availableToolApiNames) ? availableToolApiNames : [];
  const currentRound = Number.isInteger(roundState?.currentRound) ? roundState.currentRound : 0;
  const maxRounds = Number.isInteger(roundState?.maxRounds) ? roundState.maxRounds : WEB_BROWSING_MAX_ROUNDS;
  const roundStatusText = currentRound > 0 && roundState?.currentRoundHasSearch && !roundState?.currentRoundHasReader
    ? "The current round already used one search. You may read one webpage now, or start the next round with a new search if another search is needed."
    : "The next search starts a new round.";
  const availableToolsText = safeAvailableToolApiNames.length > 0
    ? safeAvailableToolApiNames.join(", ")
    : "(none)";
  ...
  return [
    ...
    "",
    "Return JSON only.",
    `Use web browsing at most ${maxRounds} rounds in the whole answer.`,
    "One round means: search once, then optionally read one webpage from that search.",
    "Call at most one web tool in each response.",
    "Each webpage read may contain only one URL.",
    `Current round: ${currentRound}/${maxRounds}.`,
    roundStatusText,
    `Available tools right now: ${availableToolsText}.`,
    "When you need a tool, return:",
    `{"action":"tool_call","identifier":"${WEB_BROWSING_IDENTIFIER}","apiName":"search","arguments":{"query":"..."}}`,
    `{"action":"tool_call","identifier":"${WEB_BROWSING_IDENTIFIER}","apiName":"crawlSinglePage","arguments":{"url":"https://example.com"}}`,
    ...
    "Use search first for broad lookups. Use crawlSinglePage for official pages, docs, pricing, release notes, rules, or verification.",
  ].join("\n");
}
...
export async function runWebBrowsingSession({
  ...
  const roundController = createWebBrowsingRoundController({ maxRounds });
  const maxPasses = getMaxWebBrowsingModelPasses(maxRounds);

  for (let pass = 0; pass < maxPasses; pass += 1) {
    if (isClientAborted?.() || signal?.aborted) break;

    const availableToolApiNames = roundController.getAvailableToolApiNames();
    if (availableToolApiNames.length === 0) break;
    ...
    const rawAction = await actionRunner({
      ...
      userText: buildToolLoopPrompt({
        ...
        roundState: roundController.getRoundState(),
      }),
    });
    ...
    if (!availableToolApiNames.includes(action.apiName)) {
      break;
    }

    const toolReservation = roundController.reserve(action.apiName);
    if (!toolReservation.allowed) {
      break;
    }
