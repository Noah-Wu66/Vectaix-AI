...
function isAgentLeaseConflictErrorMessage(message) {
  const text = typeof message === "string" ? message.trim() : "";
  return text.includes("当前任务正在执行中，请稍后再试");
}
...
async function streamExistingAgentRunInBackground({
  runId,
  conversationId,
  userId,
  messageId,
  ...
  await parseSseResponse(response, async (payload) => {
    ...
    if (data.type === "text") {
      ...
      if (isAgentLeaseConflictErrorMessage(message)) {
        throw new Error(message);
      }
      state.agentRun = {
        ...(state.agentRun || {}),
...
export async function resumeAgentRunInBackground({
  ...
  try {
    await streamExistingAgentRunInBackground({
      runId,
      conversationId,
      userId,
      messageId,
...
export async function handoffAgentRunToBackground({
  runId,
  conversationId,
  userId,
  messageId,
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (!isAgentLeaseConflictErrorMessage(error?.message)) {
        console.error("Agent handoff failed:", error?.message || error);
        return false;
      }
