...
function shouldPersistSnapshot(lastPersistAt) {
  return Date.now() - lastPersistAt >= 800;
}
  ...
  try {
    ...
    await parseSseResponse(response, async (payload) => {
      ...
      if (data.type === "text") {
        ...
        state.councilExpertStates = upsertCouncilExpertState(state.councilExpertStates, data.state);
        state.phase = "thinking";
      } else if (data.type === "council_summary_state") {
        state.councilSummaryState = data.state && typeof data.state === "object" ? data.state : null;
        state.phase = data?.state?.phase || "thinking";
