...
import { ACTIVE_CHAT_RUN_STATUSES } from "@/lib/shared/realtime";
import {
...
export const CHAT_RUN_ACTIVE_STATUSES = ACTIVE_CHAT_RUN_STATUSES;
...
async function replaceConversationMessages({
  ...
  await Promise.all([
    publishConversationUpsert({ conversationId, userId, hasActiveRun }),
    ...removedMessageIds.map((messageId) => publishMessageRemove({ conversationId, messageId })),
    ...(Array.isArray(nextMessages) ? nextMessages : []).map((message) => (
      publishMessageUpsert({ conversationId, userId, message })
    )),
  ]);
}
...
export async function createRunRequest({
  ...
  const runMode = mode === "regenerate" ? "regenerate" : "send";
  ...
  if (runMode === "regenerate" && !targetConversationId) {
    throw new Error("重新生成必须基于已有对话");
  }
  ...
  if (runMode === "regenerate") {
    if (runType !== "chat") {
      throw new Error("当前模式不支持重新生成");
    }

    let sanitizedMessages = [];
    try {
      sanitizedMessages = sanitizeStoredMessagesStrict(messages);
    } catch (error) {
      throw new Error(error?.message || "messages invalid");
    }

    if (sanitizedMessages.length === 0) {
      throw new Error("缺少可重新生成的对话内容");
    }
    if (sanitizedMessages[sanitizedMessages.length - 1]?.role !== "user") {
      throw new Error("重新生成的最后一条消息必须是用户消息");
    }
    ...
    await replaceConversationMessages({
      ...
      userId,
      nextMessages: [...sanitizedMessages, placeholderMessage],
      hasActiveRun: true,
    });
    ...
    const createdRun = await ChatRun.create({
      ...
      promptSnapshot: typeof prompt === "string" ? prompt : (typeof lastUserMessage?.content === "string" ? lastUserMessage.content : ""),
      historySnapshot: Array.isArray(history) ? history : [],
      messagesSnapshot: sanitizedMessages,
    ...
    return {
      ...
      runId: createdRun._id.toString(),
      conversationId: targetConversationId,
      userMessageId: typeof lastUserMessage?.id === "string" ? lastUserMessage.id : "",
      messageId,
      provider,
    };
  }
...
export async function patchConversationMessage({
  ...
  await Promise.all([
    publishConversationUpsert({ conversationId, userId }),
    publishMessageUpsert({
      conversationId,
      userId,
