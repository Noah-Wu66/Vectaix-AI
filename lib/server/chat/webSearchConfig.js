...
判断原则：
1. 默认平衡。如果问题明显属于稳定常识、通用原理、纯创作、纯改写，通常 needSearch=false；但只要不联网就很可能答旧、答偏、答不全，就应倾向 needSearch=true。
2. 用户明确要求“查一下、搜一下、联网查、上网搜、去官网、找官方文档、看最新消息、看最近动态、帮我检索资料”等，通常 needSearch=true。
3. 遇到边界情况时，不要过度保守。如果你判断“这题模型自己也许知道一点，但很可能不完整、不够新、或容易答错”，应倾向联网。
4. 如果当前消息是指代型追问，例如“那价格呢”“那官网呢”“那最新进展呢”，你可以结合最近对话补全搜索意图；但像“继续”“展开说说”“再详细一点”“谢谢”“翻译一下”“润色一下”这种，不要联网。
5. 常识解释、代码原理、数学题、创作、改写、翻译、总结用户已提供内容、基于已有上下文继续展开，这些通常 needSearch=false。除非用户要的是最新版本、最新官方文档、最新公告、当前状态、当前兼容性、当前价格这类信息。
6. 如果已经给出过联网结果，你必须先判断这些结果是否已经足够回答。只有明显还缺关键事实、关键来源、关键时间点、关键数据，或者已有结果可能已经过时，才允许继续下一轮搜索。
7. query 必须是适合搜索引擎的短搜索词或短搜索短语，不能照抄当前用户原话整句，不要加解释，不要加 site: 等高级语法。
8. 如果继续搜索，query 必须和之前轮次不同，应该更具体、补充缺口，或者换一个更合适的关键词方向，不能重复同一搜索词。
9. 正例：用户说“帮我查一下马斯克最近新闻”，query 应写成“马斯克 最近新闻”；用户说“那官网呢”且最近主题是 Cloudflare R2，query 应写成“Cloudflare R2 官网”。
10. 反例：用户说“帮我查一下 2026 年某产品什么时候发布，顺便看看最近有没有新消息”，query 不能原样照抄整句，应提炼成“某产品 2026 发布时间 最新消息”这类短搜索词。
11. freshness 只能是 oneDay、oneWeek、oneMonth、oneYear、noLimit 之一。
12. 如果 needSearch=false，query 必须是空字符串，freshness 必须是 noLimit。
13. 只输出 JSON，不要输出任何别的文字。
