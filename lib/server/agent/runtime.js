...
const AGENT_VERIFY_MAX_TOKENS = 1200;
...
function mapStepTypeToTimelineKind(type, toolName) {
  if (toolName === "web_search") return "search";
  if (toolName === "read_attachments") return "reader";
  if (type === "search" || type === "reader") return type;
  return "thought";
}
...
function buildVerificationPrompt({ goal, answer, searchContextText, attachmentContext }) {
  return [
    "请对下面这份 Agent 草稿做快速核对，只返回 JSON。",
    "JSON 字段必须包含：summary、evidenceLevel。",
    "summary 要用中文简短说明是否基本可靠、有没有明显遗漏。",
    "evidenceLevel 只能是 high、medium、low。",
    `任务：${goal}`,
    `草稿答案：\n${answer}`,
    searchContextText ? `联网依据：\n${clipText(searchContextText, 4000)}` : "",
    attachmentContext ? `附件依据：\n${clipText(attachmentContext, 4000)}` : "",
  ].filter(Boolean).join("\n\n");
}
...
function buildFinalPrompt({
  ...
  return [
    "你是一个成熟的办公 Agent，请基于已有资料直接给出可交付结果。",
    ...
    verificationSummary ? `核对提醒：${verificationSummary}` : "",
  ].filter(Boolean).join("\n\n");
}
...
export async function runAgentRuntime({
  ...
  emitAgentStep(sendEvent, {
    id: `agent_step_${Date.now()}_${planningStep.stepOrder}`,
    kind: "thought",
    status: "running",
    title: planningStep.title,
    message: planningStep.inputSummary,
    content: "",
  });
  ...
  emitAgentStep(sendEvent, {
    id: `agent_step_${Date.now()}_${planningStep.stepOrder}_done`,
    kind: "thought",
    status: "done",
    title: plan.planTitle,
    content: buildPlannerSummary(plan),
  });
  ...
  if (plan.shouldUseMemory) {
    ...
    run = stepIndexInfo.run;
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${memoryStep.stepOrder}`,
      kind: mapStepTypeToTimelineKind(memoryStep.type, memoryStep.toolName),
      status: "running",
      title: memoryStep.title,
      message: memoryStep.inputSummary,
    });
    ...
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${memoryStep.stepOrder}_done`,
      kind: "thought",
      status: "done",
      title: "会话记忆已读取",
      content: memorySummaries.length > 0 ? memorySummaries.join("\n") : "没有可用的近期记忆",
    });
  }
  ...
  if (plan.shouldSearch && enableWebSearch) {
    ...
    run = stepIndexInfo.run;
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${searchStep.stepOrder}`,
      kind: "search",
      status: "running",
      title: searchStep.title,
      query: searchStep.inputSummary,
    });
    ...
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${searchStep.stepOrder}_done`,
      kind: "search",
      status: "done",
      title: "联网搜索已完成",
      content: searchContextText ? clipText(searchContextText, 1000) : "联网没有拿到有效补充资料",
    });
  }
  ...
  if (plan.shouldCompute) {
    ...
    run = stepIndexInfo.run;
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${computeStep.stepOrder}`,
      kind: "thought",
      status: "running",
      title: computeStep.title,
      message: computeStep.inputSummary,
    });
    ...
    emitAgentStep(sendEvent, {
      id: `agent_step_${Date.now()}_${computeStep.stepOrder}_done`,
      kind: "thought",
      status: "done",
      title: "结构化整理已完成",
      content: computeContext || "没有可整理的结构化结果",
    });
  }
  ...
  emitAgentStep(sendEvent, {
    id: `agent_step_${Date.now()}_${draftStep.stepOrder}`,
    kind: "thought",
    status: "running",
    title: draftStep.title,
    message: draftStep.inputSummary,
  });
  ...
  const verifyStep = {
    ...
    status: "running",
    toolName: null,
    inputSummary: "检查答案是否和资料一致",
    outputSummary: "",
    startedAt: new Date(),
    finishedAt: null,
  };
  stepIndexInfo = await createRunStep(run, verifyStep);
  run = stepIndexInfo.run;
  emitAgentStep(sendEvent, {
    id: `agent_step_${Date.now()}_${verifyStep.stepOrder}`,
    kind: "thought",
    status: "running",
    title: verifyStep.title,
    message: verifyStep.inputSummary,
  });

  let verificationSummary = "";
  try {
    const verifyRequestBody = buildSeedJsonRequestBody({
      model: SEED_MODEL_ID,
      input: [
        buildSeedMessageInput({
          role: "user",
          content: [{ type: "input_text", text: buildVerificationPrompt({
            goal: prompt || run.goal,
            answer: draftAnswer,
            searchContextText,
            attachmentContext,
      ...
      instructions: await injectCurrentTimeSystemReminder("你是 Agent 的核对器，只返回 JSON。"),
      maxTokens: AGENT_VERIFY_MAX_TOKENS,
      temperature: 0.1,
    });
    const verifyResult = await requestSeedJson({ apiKey, requestBody: verifyRequestBody, req });
    const verifyParsed = parseJsonFromText(verifyResult.text) || {};
    verificationSummary = typeof verifyParsed.summary === "string" ? verifyParsed.summary.trim() : "";
  } catch {
    verificationSummary = "";
  }
  ...
  run = await patchRunStep(run, stepIndexInfo.stepIndex, {
    status: "done",
    outputSummary: verificationSummary || "已完成基础核对",
    finishedAt: new Date(),
  });
  emitAgentStep(sendEvent, {
    id: `agent_step_${Date.now()}_${verifyStep.stepOrder}_done`,
    kind: "thought",
    status: "done",
    title: "结果核对已完成",
    content: verificationSummary || "已完成基础核对",
  });
  ...
  const finalAnswer = verificationSummary
    ? `${draftAnswer}\n\n核对说明：${verificationSummary}`
    : draftAnswer;
