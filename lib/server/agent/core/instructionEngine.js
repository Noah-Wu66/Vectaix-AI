...
import { isDocumentCategory, isNativeBinaryCategory } from "@/lib/shared/attachments";
...
function countPlannableAttachments(historyMessages, currentAttachments) {
  ...
  for (const item of Array.isArray(currentAttachments) ? currentAttachments : []) {
    if (typeof item?.url === "string" && item.url && isDocumentCategory(item?.category)) urls.add(item.url);
  }
  for (const message of Array.isArray(historyMessages) ? historyMessages : []) {
    for (const part of Array.isArray(message?.parts) ? message.parts : []) {
      if (
        ...
        && part.fileData.url
        && isDocumentCategory(part?.fileData?.category)
      ) {
...
function buildPlanDecision({ prompt, attachmentCount, enableWebSearch }) {
  return buildPlannerFallback({ prompt, attachmentCount, enableWebSearch });
}
...
function buildToolLoopPrompt({
  ...
  if (allowSandbox) {
    availableActions.push(
      '{"instruction":"call_tool","toolCall":{"identifier":"vectaix-vercel-sandbox","apiName":"exec","arguments":{"command":"<命令>","cwd":"/vercel/sandbox/vectaix"},"type":"builtin"}}',
      '{"instruction":"call_tool","toolCall":{"identifier":"vectaix-vercel-sandbox","apiName":"uploadBlob","arguments":{"url":"<blob_url>","remotePath":"<路径>"},"type":"builtin"}}',
      '{"instruction":"call_tool","toolCall":{"identifier":"vectaix-vercel-sandbox","apiName":"readFile","arguments":{"path":"<路径>"},"type":"builtin"}}',
      '{"instruction":"call_tool","toolCall":{"identifier":"vectaix-vercel-sandbox","apiName":"downloadArtifact","arguments":{"path":"<路径>","title":"<标题>","mimeType":"<MIME>","extension":"<扩展名>"},"type":"builtin"}}'
    );
  }
  if (allowWebSearch) {
    availableActions.push(
      '{"instruction":"call_tool","toolCall":{"identifier":"lobe-web-browsing","apiName":"search","arguments":{"query":"<搜索词>","searchCategories":["general"],"searchTimeRange":"day"},"type":"builtin"}}',
      '{"instruction":"call_tool","toolCall":{"identifier":"lobe-web-browsing","apiName":"crawlSinglePage","arguments":{"url":"<网址>"},"type":"builtin"}}',
      '{"instruction":"call_tool","toolCall":{"identifier":"lobe-web-browsing","apiName":"crawlMultiPages","arguments":{"urls":["<网址1>","<网址2>"]},"type":"builtin"}}'
    );
  }
  ...
  return [
    ...
    "严格只返回 JSON，每次只返回一个 instruction。",
    "只允许两种 instruction：call_tool 或 finish。",
    "如果已经拿到足够信息，必须返回 finish。",
    `当前进度：第 ${round + 1} 轮 / 共 ${maxRounds} 轮（剩余 ${remainingRounds} 轮）。${remainingRounds <= 1 ? "这是最后一轮，如果信息基本够用请直接 finish。" : ""}`,
    "可选 JSON 示例：",
    ...availableActions,
    '{"instruction":"finish","answer":"..."}',
    "错误处理规则：如果上一次工具调用失败了，不要用相同的参数重试。换一个搜索词、换一个 URL、或者直接 finish 用已有信息回答。",
...
function buildFinalPrompt({
  ...
  const outputStyle = typeof plan?.outputStyle === "string" && plan.outputStyle.trim()
    ? plan.outputStyle.trim()
    : "用简体中文，大白话，结论优先。";
  return [
    "你是 Vectaix 的 Agent，请基于当前请求内已经拿到的资料直接给出可交付结果。",
    "如果用户问你是谁、你是什么、你来自哪里，必须明确回答你是 Vectaix 的 Agent。",
    `要求：${outputStyle}如果用了联网信息，要尽量体现来源；如果资料不足，要明确说不确定。`,
...
export async function runInstructionEngine({
  ...
  const documentAttachments = Array.isArray(attachments)
    ? attachments.filter((item) => isDocumentCategory(item?.category))
    : [];
  const mediaAttachments = Array.isArray(attachments)
    ? attachments.filter((item) => isNativeBinaryCategory(item?.category))
    : [];
  ...
  const plan = buildPlanDecision({
    prompt: goal,
    attachmentCount: countPlannableAttachments(historyMessages, documentAttachments),
    enableWebSearch,
  });
  ...
  if (plan.shouldReadAttachments && documentAttachments.length > 0) {
    const attachmentStep = coordinator.startStep({
      kind: "reader",
      title: "正在读取附件资料",
      content: `${documentAttachments.length} 个附件`,
    });
    ...
    for (let round = 0; round < AGENT_TOOL_LOOP_MAX_ROUNDS; round += 1) {
      ...
      const instruction = await requestNextInstruction({
        ...
        prompt: buildToolLoopPrompt({
          ...
          allowWebSearch,
          round,
          maxRounds: AGENT_TOOL_LOOP_MAX_ROUNDS,
        }),
        thinkingLevel,
        allowSandbox,
        allowWebSearch,
