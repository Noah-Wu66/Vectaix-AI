...
function buildChatCompletionsHeaders(apiKey) {
  const headers = {
    "Content-Type": "application/json",
  };
...
function normalizeChatCompletionReasoningDelta(delta) {
...
async function streamMiMoAnswer({ req, historyMessages, prompt, images, preparedAttachments, userId, instructions, sendEvent }) {
  assertImageSupport(MIMO_V2_FLASH_MODEL, images, historyMessages);
  const providerConfig = resolveMiMoProviderConfig();
  const fileTextMap = await getPreparedAttachmentTextsByUrls(collectFileUrlsFromMessages(historyMessages), { userId });
  ...
  const response = await fetch(`${providerConfig.baseUrl}/chat/completions`, {
    method: "POST",
    headers: buildChatCompletionsHeaders(providerConfig.apiKey),
    body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(await response.text() || "MiMo 请求失败");
  }
  ...
  await readSseStream({
    response,
    signal: req?.signal,
    emptyBodyMessage: "MiMo 返回了空响应体",
    onEvent: (event) => {
      const choice = event?.choices?.[0];
      const delta = choice?.delta;
      const thought = normalizeChatCompletionReasoningDelta(delta);
      if (thought) {
        sendEvent({ type: "thought", content: thought });
      }
      const text = normalizeOpenAIChunkText(delta?.content);
...
export async function runAgentControlText({
  ...
  if (provider === "xiaomi") {
    ...
    if (!response.ok) {
      throw new Error(await response.text() || "MiMo 控制请求失败");
    }
...
export async function streamAgentFinalAnswer({ apiKey, req, driverModel, historyMessages, prompt, images, preparedAttachments, userId, instructions, thinkingLevel, sendEvent }) {
  ...
  if (provider === "xiaomi") {
    return streamMiMoAnswer({ req, historyMessages, prompt, images, preparedAttachments, userId, instructions, sendEvent });
  }
  if (provider === "deepseek") {
