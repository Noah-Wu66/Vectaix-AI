...
import {
  ...
  resolveGeminiProviderConfigForUser,
...
function mapOpenRouterReasoningEffort(value, modelId) {
  const normalized = typeof value === "string" && value ? value.trim().toLowerCase() : "";
  const fallback = typeof getDefaultThinkingLevel(modelId) === "string"
    ? getDefaultThinkingLevel(modelId).trim().toLowerCase()
    : "high";
  const source = normalized || fallback;
  if (source === "minimal" || source === "none" || source === "low") return "low";
  if (source === "medium") return "medium";
  return "high";
}
...
async function storedPartToOpenRouterPart(part, role, fileTextMap) {
  if (!part || typeof part !== "object") return null;
...
function normalizeOpenRouterMessageContent(role, parts) {
  ...
  const hasRichPart = parts.some((part) => part?.type !== "text");
  if (!hasRichPart) {
    const text = parts
      .map((part) => (part?.type === "text" ? part.text : ""))
      .filter(Boolean)
...
async function buildOpenRouterMessagesFromHistory(messages, fileTextMap) {
  const result = [];
  for (const msg of messages) {
    if (msg?.role !== "user" && msg?.role !== "model") continue;
    const storedParts = getStoredPartsFromMessage(msg);
    ...
    const contentParts = [];
    for (const storedPart of storedParts) {
      const part = await storedPartToOpenRouterPart(storedPart, role, fileTextMap);
      if (part) contentParts.push(part);
    }
    const content = normalizeOpenRouterMessageContent(role, contentParts);
    if (content) {
...
async function streamOpenAIAnswer({ req, model, historyMessages, prompt, images, preparedAttachments, userId, instructions, thinkingLevel, sendEvent }) {
  ...
  if (providerConfig.transport === "openrouter-chat") {
    const messages = await buildOpenRouterMessagesFromHistory(historyMessages, fileTextMap);
    const userContent = [];
    if (userText) {
    ...
    const content = normalizeOpenRouterMessageContent("user", userContent);
    if (content) {
      messages.push({ role: "user", content });
    }
    const response = await requestOpenRouterChatCompletion({
      apiKey: providerConfig.apiKey,
      model: getOpenRouterModelId(model) || model,
      messages: [
      ...
      reasoningEffort: getModelConfig(model)?.supportsThinkingLevelControl === true
        ? mapOpenRouterReasoningEffort(thinkingLevel, model)
        : undefined,
      signal: req?.signal,
    });
...
async function streamClaudeAnswer({ req, model, historyMessages, prompt, images, preparedAttachments, userId, instructions, thinkingLevel, sendEvent }) {
  assertImageSupport(model, images, historyMessages);
  const fileTextMap = await getPreparedAttachmentTextsByUrls(collectFileUrlsFromMessages(historyMessages), { userId });
  const providerConfig = await resolveAnthropicProviderConfig(model, userId);
  const apiModel = resolveAnthropicApiModel(model, providerConfig);
  const userText = buildCurrentUserText({ prompt, preparedAttachments });
...
async function streamGeminiAnswer({ req, model, historyMessages, prompt, images, preparedAttachments, userId, instructions, thinkingLevel, sendEvent }) {
  ...
  if (providerConfig.transport === "openrouter-chat") {
    ...
    const response = await requestOpenRouterChatCompletion({
      apiKey: providerConfig.apiKey,
      model: resolveGeminiApiModel(model, providerConfig),
      messages: [
        { role: "system", content: instructions },
        ...messages,
      ],
  ...
  const ai = await createGeminiClient(userId);
  const config = {
...
export async function runAgentControlText({
  ...
  if (provider === "openai") {
    ...
    if (providerConfig.transport === "openrouter-chat") {
      const response = await requestOpenRouterChatCompletion({
        apiKey: providerConfig.apiKey,
        model: getOpenRouterModelId(normalizedDriverModel) || normalizedDriverModel,
        messages: [
        ...
        maxTokens,
        temperature,
        reasoningEffort: getModelConfig(normalizedDriverModel)?.supportsThinkingLevelControl === true
          ? mapOpenRouterReasoningEffort(thinkingLevel, normalizedDriverModel)
          : undefined,
        signal: req?.signal,
      });
  ...
  if (isAnthropicCompatibleProvider(provider)) {
    const providerConfig = await resolveAnthropicProviderConfig(normalizedDriverModel, userId);
    const apiModel = resolveAnthropicApiModel(normalizedDriverModel, providerConfig);
    if (providerConfig.transport === "openrouter-chat") {
      const response = await requestOpenRouterChatCompletion({
        apiKey: providerConfig.apiKey,
        model: apiModel,
  ...
  if (provider === "gemini") {
    ...
    if (providerConfig.transport === "openrouter-chat") {
      const response = await requestOpenRouterChatCompletion({
        apiKey: providerConfig.apiKey,
        model: resolveGeminiApiModel(normalizedDriverModel, providerConfig),
        messages: [
          { role: "system", content: systemPrompt },
          { role: "user", content: userText },
        ],
