...
function buildToolLoopPrompt({
  ...
  return [
    ...
    '5. {"action":"tool_call","tool":"lobe-web-browsing.search","input":{"query":"OpenAI Responses API docs","searchCategories":["general"],"searchTimeRange":"day"}}',
    '6. {"action":"tool_call","tool":"lobe-web-browsing.crawlSinglePage","input":{"url":"https://example.com"}}',
    '7. {"action":"tool_call","tool":"lobe-web-browsing.crawlMultiPages","input":{"urls":["https://example.com","https://example.org"]}}',
    '8. {"action":"final_answer","answer":"..."}',
...
async function requestToolAction({ apiKey, req, userId, driverModel, prompt, thinkingLevel }) {
  const resultText = await runAgentControlText({
    apiKey,
    req,
    userId,
    driverModel,
    systemPrompt: await injectCurrentTimeSystemReminder("你是 Vectaix Agent 的同步 JSON 工具调度器，只能输出 JSON。"),
    userText: prompt,
    maxTokens: 900,
    thinkingLevel,
    temperature: 0.1,
  });
  return normalizeToolAction(resultText);
}

function buildSearchEventResults(results) {
...
function buildPageEventResults(results) {
  if (!Array.isArray(results)) return [];
  return results
    .slice(0, 5)
    .map((item) => {
...
async function executeToolCall({
  ...
  if (toolName === `${WEB_BROWSING_IDENTIFIER}.${WebBrowsingApiName.search}`) {
    const query = typeof input?.query === "string" ? input.query.trim() : "";
    if (!query) throw new Error("lobe-web-browsing.search 缺少 query");
    const eventRound = Date.now() + Math.floor(Math.random() * 1000);
    ...
    const args = {
      query,
      searchCategories: Array.isArray(input?.searchCategories)
        ? input.searchCategories.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
        : undefined,
      searchEngines: Array.isArray(input?.searchEngines)
        ? input.searchEngines.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
        : undefined,
      searchTimeRange: typeof input?.searchTimeRange === "string" ? input.searchTimeRange.trim() : undefined,
    };

    const result = await webBrowsingRuntime.search(args, { signal });
    const webBrowsingToolCall = {
    ...
    if (result.success) {
      pushUniqueCitations(
        citations,
        Array.isArray(result?.state?.results)
          ? result.state.results.map((item) => ({
            url: item.url,
            title: item.title || item.url,
      ...
      sendEvent?.({
        ...
        round: eventRound,
        query,
        results: buildSearchEventResults(result?.state?.results),
      });
    } else {
      sendEvent?.({
        type: "search_error",
    ...
    return {
      ...
      toolEvent: {
        ...
        title: "已执行联网搜索",
        summary: clipText(`${query}\n${result?.success ? `共 ${result?.state?.resultNumbers || 0} 条结果` : (result?.content || "搜索失败")}`, AGENT_TOOL_RESULT_MAX_CHARS),
      },
    };
  }

  if (toolName === `${WEB_BROWSING_IDENTIFIER}.${WebBrowsingApiName.crawlSinglePage}` || toolName === `${WEB_BROWSING_IDENTIFIER}.${WebBrowsingApiName.crawlMultiPages}`) {
    const urls = toolName === `${WEB_BROWSING_IDENTIFIER}.${WebBrowsingApiName.crawlSinglePage}`
      ? [typeof input?.url === "string" ? input.url.trim() : ""].filter(Boolean)
      : (Array.isArray(input?.urls)
        ? input.urls.filter((item) => typeof item === "string" && item.trim()).map((item) => item.trim())
        : []);
    if (urls.length === 0) {
    ...
    const result = toolName === `${WEB_BROWSING_IDENTIFIER}.${WebBrowsingApiName.crawlSinglePage}`
      ? await webBrowsingRuntime.crawlSinglePage({ url: urls[0] }, { signal })
      : await webBrowsingRuntime.crawlMultiPages({ urls }, { signal });

    const webBrowsingToolCall = {
      identifier: WEB_BROWSING_IDENTIFIER,
      apiName: toolName.endsWith(WebBrowsingApiName.crawlSinglePage)
        ? WebBrowsingApiName.crawlSinglePage
        : WebBrowsingApiName.crawlMultiPages,
      arguments: toolName.endsWith(WebBrowsingApiName.crawlSinglePage) ? { url: urls[0] } : { urls },
      content: result.content,
      success: result.success,
      state: result.state,
    };
    ...
    if (result.success) {
      ...
      sendEvent?.({
        ...
        url: urls[0],
        urls,
        results: buildPageEventResults(result?.state?.results),
      });
    } else {
      sendEvent?.({
        type: "search_error",
    ...
    return {
      ...
      toolEvent: {
        id: `tool_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        title: toolName.endsWith(WebBrowsingApiName.crawlSinglePage) ? "已抓取网页" : "已抓取多个网页",
        summary: clipText(`${urls.join("\n")}\n${result?.success ? `共 ${Array.isArray(result?.state?.results) ? result.state.results.length : 0} 页` : (result?.content || "抓取失败")}`, AGENT_TOOL_RESULT_MAX_CHARS),
      },
    };
  }
...
export async function runAgentRuntimeV2({
  ...
  const webBrowsingRuntime = new WebBrowsingExecutionRuntime({
    searchService: new SearchService({ webSearchOptions: config?.webSearch }),
  });
  ...
  if (plan.shouldUseSandbox || (plan.shouldSearch && enableWebSearch === true)) {
    emitAgentStep(sendEvent, {
      id: `agent_tool_${Date.now()}_start`,
      kind: "tool",
      status: "running",
      title: "正在执行同步工具操作",
      content: "会按需要调用联网工具或沙盒工具，并在当前请求里完成",
    });
    ...
    for (let round = 0; round < AGENT_TOOL_LOOP_MAX_ROUNDS; round += 1) {
      ...
      if (executed?.webBrowsingToolCall) {
        webBrowsingToolCalls = [...webBrowsingToolCalls, executed.webBrowsingToolCall];
