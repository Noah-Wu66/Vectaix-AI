...
async function buildPlannerDecision({ apiKey, req, driverModel, prompt, historyMessages, attachmentCount, enableWebSearch, thinkingLevel }) {
...
async function runDriverDecision({ apiKey, req, driverModel, prompt, historyMessages, searchRounds, thinkingLevel }) {
...
async function requestToolAction({ apiKey, req, driverModel, prompt, thinkingLevel }) {
  const resultText = await runAgentControlText({
    apiKey,
    req,
    driverModel,
...
export async function runAgentRuntimeV2({
  ...
  if (plan.shouldSearch && enableWebSearch === true) {
    ...
    const citationList = [];
    const driverProvider = getModelProvider(driverModel);
    const searchResult = await runWebSearchOrchestration({
      ...
      signal: req?.signal,
      ...getWebSearchProviderRuntimeOptions(driverProvider),
    });
