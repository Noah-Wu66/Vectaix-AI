...
async function buildFinalSeedAnswer({ apiKey, req, model, instructions, historyMessages, prompt, images, thinkingLevel }) {
...
export async function runAgentRuntimeV2({
  ...
  const agentThinkingLevel = config.thinkingLevel.trim().toLowerCase();
  ...
  try {
    ...
    while ((Number(run.currentCursor) || 0) < AGENT_STEP_SEQUENCE.length) {
      ...
      if (stepType === "search_decide") {
        ...
        const decisionText = await runSeedDecision({
          apiKey,
          req,
          prompt: goal,
          historyMessages,
