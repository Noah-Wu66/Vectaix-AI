...
const DEFAULT_TEMPLATE_REF = "vectaix-agent-dev";
const DEFAULT_TEMPLATE_VERSION = "dev";
...
function buildTemplateRef() {
  return process.env.E2B_TEMPLATE || DEFAULT_TEMPLATE_REF;
}

export function getE2BTemplateVersion() {
  return process.env.E2B_TEMPLATE_VERSION || DEFAULT_TEMPLATE_VERSION;
}
...
export async function createOrConnectSandbox({
  ...
  if (!sandbox) {
    sandbox = await Sandbox.create(buildTemplateRef(), {
