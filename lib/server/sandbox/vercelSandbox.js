...
const PARSER_BOOTSTRAP_VERSION = "2026-03-14-system-python-deps";
...
async function runAbortableTask({ signal, onAbort, task }) {
  if (!signal) return task();
  if (signal.aborted) {
    if (typeof onAbort === "function") {
      await Promise.resolve(onAbort()).catch(() => {});
    }
...
async function ensureBaseDirectories(sandbox, workdir) {
  const command = await sandbox.runCommand({
    ...
    args: [
      "-lc",
      `mkdir -p ${encodeShell(path.posix.dirname(PARSER_SCRIPT_REMOTE_PATH))} ${encodeShell(workdir)} ${encodeShell(`${workdir}/uploads`)} ${encodeShell(`${workdir}/artifacts`)}`,
    ],
  });
...
async function installSandboxPackages(sandbox, packages, errorMessage, { optional = false } = {}) {
  ...
  if (installCommand.exitCode === 0 || optional) {
    if (installCommand.exitCode !== 0) {
      const stderr = await installCommand.stderr().catch(() => "");
      console.warn(errorMessage, stderr || "no stderr");
    }
    return;
  }
  const stderr = await installCommand.stderr().catch(() => "");
...
async function ensureParserBootstrap(sandbox) {
  ...
  const existingVersion = existing ? (await readStreamToBuffer(existing)).toString("utf8").trim() : "";
  if (existingVersion === PARSER_BOOTSTRAP_VERSION) return;
  ...
  if (antiwordCheck.exitCode !== 0) {
    await installSandboxPackages(sandbox, ["antiword"], "antiword 安装失败，旧版 DOC 解析将不可用", { optional: true });
  }

  await installSandboxPackages(
    sandbox,
    ["python3-pypdf", "python3-docx", "python3-openpyxl", "python3-xlrd"],
    "安装 Python 文档解析依赖失败"
  );
  ...
  await sandbox.writeFiles([
    ...
    { path: PARSER_BOOTSTRAP_MARKER, content: Buffer.from(PARSER_BOOTSTRAP_VERSION, "utf8") },
  ]);
}
...
export async function runSandboxCommand({
  ...
  const result = await runAbortableTask({
    signal,
    onAbort: async () => {
      await sandbox.stop().catch(() => {});
    },
    task: () => sandbox.runCommand({
      cmd: "bash",
      args: ["-lc", command],
      cwd: workdir,
    }),
...
export async function parseAttachmentInSandbox({
  ...
  try {
    ...
    const command = await runAbortableTask({
      signal,
      onAbort: async () => {
        await sandbox.stop().catch(() => {});
      },
...
export async function downloadSandboxArtifactToBlob({
  ...
  const pathname = `agent/${String(userId)}/${String(conversationId || "detached")}/manual/${safeTitle}.${extension}`;
  const blob = await put(pathname, buffer, {
    access: "public",
