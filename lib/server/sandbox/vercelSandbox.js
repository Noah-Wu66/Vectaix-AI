...
async function ensureParserBootstrap(sandbox) {
  ...
  if (installAntiword.exitCode !== 0) {
    ...
    throw new Error(stderr || "安装 antiword 失败");
  }
...
function buildRemoteInputPath({ conversationId, originalName, purpose = "agent" }) {
  return `${getSandboxWorkdir(conversationId, purpose)}/uploads/${sanitizeSegment(originalName, "file")}`;
}

function buildRemoteOutputPath({ conversationId, originalName }) {
  const baseName = sanitizeSegment(originalName.replace(/\.[^.]+$/, ""), "file");
  return `${getSandboxWorkdir(conversationId, "parser")}/artifacts/${baseName}.json`;
}
...
export async function parseAttachmentInSandbox({
  ...
  try {
    ...
    const inputPath = buildRemoteInputPath({ conversationId: sandboxConversationId, originalName, purpose: "parser" });
    const outputPath = buildRemoteOutputPath({ conversationId: sandboxConversationId, originalName });
    const response = await fetch(blobFile.url, { cache: "no-store" });
    if (!response.ok) {
      throw new Error("文件下载失败");
    }
