...
const DEFAULT_SEED_API_TIMEOUT_MS = 30_000;
...
export function createSeedUpstreamSignal(req, timeoutMs = DEFAULT_SEED_API_TIMEOUT_MS) {
  const timeoutSignal = Number.isFinite(timeoutMs) && timeoutMs > 0
    ? AbortSignal.timeout(timeoutMs)
    : null;
  if (req?.signal && timeoutSignal) {
    return AbortSignal.any([req.signal, timeoutSignal]);
  }
  return req?.signal || timeoutSignal;
}
...
export async function requestSeedResponses({
  ...
  requestBody,
  req,
  timeoutMs = DEFAULT_SEED_API_TIMEOUT_MS,
}) {
  ...
  for (let attempt = 0; attempt < SEED_MAX_RETRIES; attempt += 1) {
    try {
      const response = await fetch(url, {
        ...
        signal: createSeedUpstreamSignal(req, timeoutMs),
      });
...
export async function requestSeedJson({
  ...
  const response = await requestSeedResponses({ apiKey, requestBody, req, timeoutMs });
