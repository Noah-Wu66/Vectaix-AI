...
export function createChatAppActions({
  ...
  const handleSendFromComposer = async ({ text, attachments, imageGenSize, imageGenResolution }) => {
        ...
        if (isImageAttachment(attachment)) {
          ...
          if (typeof mimeType === "string" && mimeType) {
            ...
            displayImages.push({
              url: blobUrl,
              mimeType,
              ...(previewUrl && previewUrl !== blobUrl ? { localPreviewUrl: previewUrl } : {}),
            });
          }
          continue;
        }
    ...
    setLoading(true);
    try {
      const config = isImageGenModel(model)
        ? buildChatConfig({
            modelId: model,
        ...
        : buildRuntimeConfig({ images: uploadedImages, attachments: uploadedFiles });
      await runChat({
        prompt: text,
  ...
  const regenerateModelMessage = async (index) => {
    ...
    try {
      await runChat({
        ...
        conversationId: currentConversationId,
        model,
        config: isImageGenModel(model)
          ? buildChatConfig({ modelId: model })
          : buildRuntimeConfig(),
        historyLimit,
        currentConversationId,
        setCurrentConversationId,
