    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        for (const attachment of attachments) {
          ...
          // 标记解析完成
          const visualAssetCount = Number(prepared?.file?.visualAssetCount) || 0;
          updatePrepareTimeline((prev) =>
            prev.map((s) => (s.id === parseStepId
              ? {
                ...s,
                status: "done",
                title: `「${fileName}」已解析`,
                message: `「${fileName}」已解析${visualAssetCount > 0 ? `，提取到 ${visualAssetCount} 张图` : ""}`,
              }
              : s))
          );
        ...
        setMessages((prev) => {
          ...
          for (let i = next.length - 1; i >= 0; i -= 1) {
            ...
            if (m?.role === "user" && m?.id === userMsg.id) {
              ...
              // 合并 prepared 返回的 file 和原始附件的 size（服务端可能未保存到 size）
              const originalSizeMap = new Map();
              if (Array.isArray(attachments)) {
                for (const att of attachments) {
                  if (att?.blobUrl && att?.size) originalSizeMap.set(att.blobUrl, att.size);
                }
              }
              for (const file of preparedAttachments) {
                if (file?.url && file?.name && file?.mimeType && file?.extension && file?.category) {
                  const size = (file.size && file.size > 0) ? file.size : (originalSizeMap.get(file.url) || 0);
                  nextParts.push({ fileData: { ...file, size } });
                }
              }
