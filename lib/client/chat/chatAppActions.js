    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        for (const attachment of attachments) {
          ...
          const isDoc = !isImageAttachment(attachment);

          // 显示上传步骤
          const uploadStepId = `upload_${Date.now()}_${Math.random()}`;
          if (isDoc) {
            updatePrepareTimeline((prev) => [
              ...prev,
              { id: uploadStepId, kind: "upload", status: "running", title: `正在上传「${fileName}」`, message: `正在上传「${fileName}」`, synthetic: false },
            ]);
          }
          ...
          if (isDoc) {
            updatePrepareTimeline((prev) =>
              prev.map((s) => (s.id === uploadStepId ? { ...s, status: "done", title: `「${fileName}」已上传`, message: `「${fileName}」已上传` } : s))
            );
          }
