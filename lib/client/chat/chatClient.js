...
function resolveDirectChatEndpoint(modelId) {
  if (isFusionModel(modelId)) return "/api/fusion";
...
export async function runChat({
  ...
  const provider = isFusionModel(model) ? "fusion" : modelProvider;
  ...
  const fusionHistory = isFusionModel(model) ? [] : historyPayload;
  ...
  const shouldDelayConversationActivation = provider === "fusion";
  let conversationActivated = false;
  let hasFusionRuntimeProgress = false;
  let preserveFusionErrorState = false;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // 检测上下文超出错误，自动触发压缩重试（Fusion 模式有自己的历史管理机制，不触发压缩）
      if (!_isCompressedRetry && isContextOverflowError(errorMessage) && historyMessages.length > 0 && provider !== "fusion") {
    ...
    setMessages((prev) => {
      ...
      const nextStreamingState = {
        ...
        artifacts: null,
        fusionExperts: null,
        fusionExpertStates: null,
        fusionAnalysis: null,
        fusionAnalysisState: null,
        fusionResultState: null,
        searchError: null,
      };
    ...
    let citations = null;
    let fusionExperts = null;
    let fusionExpertStates = null;
    let fusionAnalysis = null;
    let fusionAnalysisState = null;
    let fusionResultState = null;
    ...
    const upsertFusionExpertState = (nextState) => {
      ...
      const base = Array.isArray(fusionExpertStates) ? fusionExpertStates : [];
      const index = base.findIndex((item) => item?.key === key);
      if (index >= 0) {
        const next = base.slice();
        next[index] = { ...next[index], ...nextState, key };
        fusionExpertStates = next;
        return;
      }
      fusionExpertStates = [...base, { ...nextState, key }];
    };
    ...
    const flushStreamingMessage = () => {
      ...
      setMessages((prev) => {
        ...
        const nowHasContent =
          ...
          isSearching ||
          searchError ||
          (Array.isArray(fusionExpertStates) && fusionExpertStates.length > 0) ||
          (fusionAnalysis && typeof fusionAnalysis === "object") ||
          (fusionAnalysisState && typeof fusionAnalysisState === "object") ||
          (fusionResultState && typeof fusionResultState === "object") ||
        ...
        if (
          ...
          base.fusionExperts === nextMsg.fusionExperts &&
          base.fusionExpertStates === nextMsg.fusionExpertStates &&
          base.fusionAnalysis === nextMsg.fusionAnalysis &&
          base.fusionAnalysisState === nextMsg.fusionAnalysisState &&
          base.fusionResultState === nextMsg.fusionResultState &&
    ...
    const applyEventPayload = (payload) => {
      ...
      try {
        ...
        if (data.type === "thought") {
          ...
          scheduleFlush();
        } else if (data.type === "fusion_experts") {
          hasFusionRuntimeProgress = true;
          fusionExperts = Array.isArray(data.experts) ? data.experts : null;
          scheduleFlush();
        } else if (data.type === "fusion_expert_result") {
          if (data.expert && typeof data.expert === "object") {
            hasFusionRuntimeProgress = true;
            if (!fusionExperts) fusionExperts = [];
            const idx = fusionExperts.findIndex((e) => e.label === data.expert.label);
            if (idx >= 0) {
              fusionExperts = fusionExperts.map((e, i) => i === idx ? data.expert : e);
            } else {
              fusionExperts = [...fusionExperts, data.expert];
            }
            scheduleFlush();
          }
        } else if (data.type === "fusion_expert_states") {
          hasFusionRuntimeProgress = true;
          fusionExpertStates = Array.isArray(data.experts) ? data.experts : null;
          scheduleFlush();
        } else if (data.type === "fusion_expert_state") {
          hasFusionRuntimeProgress = true;
          upsertFusionExpertState(data.expert);
          scheduleFlush();
        } else if (data.type === "fusion_analysis_state") {
          hasFusionRuntimeProgress = true;
          fusionAnalysisState = data.analysis && typeof data.analysis === "object"
            ? data.analysis
            : null;
          scheduleFlush();
        } else if (data.type === "fusion_analysis_result") {
          hasFusionRuntimeProgress = true;
          fusionAnalysis = data.analysis && typeof data.analysis === "object"
            ? data.analysis
            : null;
          scheduleFlush();
        } else if (data.type === "fusion_result_state") {
          hasFusionRuntimeProgress = true;
          fusionResultState = data.result && typeof data.result === "object"
            ? data.result
            : null;
          scheduleFlush();
          ...
          scheduleFlush();
        } else if (data.type === "fusion_triage") {
          if (data.skipped) {
            // Seed 预判为简单问题，将所有专家状态设为 skipped
            const base = Array.isArray(fusionExpertStates) ? fusionExpertStates : [];
            fusionExpertStates = base.map((item) => ({
              ...item,
              status: "skipped",
              phase: "skipped",
              message: "已跳过",
    ...
    const hasFusionProgress =
      (Array.isArray(fusionExpertStates) && fusionExpertStates.length > 0)
      || (fusionAnalysis && typeof fusionAnalysis === "object")
      || (fusionAnalysisState && typeof fusionAnalysisState === "object")
      || (fusionResultState && typeof fusionResultState === "object");
    if (streamErrorMessage && fullText.trim() === "" && !hasFusionProgress) {
      // 移除正在流式输出的空消息
      if (streamMsgId !== null) {
        ...
        try {
          ...
          setMessages((prev) => {
            ...
            const streamArtifacts = Array.isArray(streamMsg?.artifacts)
              ? streamMsg.artifacts
              : null;
            const streamFusionExperts = Array.isArray(streamMsg?.fusionExperts)
              ? streamMsg.fusionExperts
              : null;
            const streamCitations = Array.isArray(streamMsg?.citations)
              ? streamMsg.citations
              : null;
            const streamFusionExpertStates = Array.isArray(streamMsg?.fusionExpertStates)
              ? streamMsg.fusionExpertStates
              : null;
            const streamFusionAnalysis = streamMsg?.fusionAnalysis && typeof streamMsg.fusionAnalysis === "object"
              ? streamMsg.fusionAnalysis
              : null;
            const streamFusionAnalysisState = streamMsg?.fusionAnalysisState && typeof streamMsg.fusionAnalysisState === "object"
              ? streamMsg.fusionAnalysisState
              : null;
            const streamFusionResultState = streamMsg?.fusionResultState && typeof streamMsg.fusionResultState === "object"
              ? streamMsg.fusionResultState
              : null;
              ...
              for (let i = next.length - 1; i >= 0; i -= 1) {
                if (next[i]?.role === "model") {
                  next[i] = {
                    ...
                    ...(streamFusionExperts?.length ? { fusionExperts: streamFusionExperts } : {}),
                    ...(streamFusionAnalysis ? { fusionAnalysis: streamFusionAnalysis } : {}),
                    ...(streamFusionExpertStates?.length ? { fusionExpertStates: streamFusionExpertStates } : {}),
                    ...(streamFusionAnalysisState ? { fusionAnalysisState: streamFusionAnalysisState } : {}),
                    ...(streamFusionResultState ? { fusionResultState: streamFusionResultState } : {}),
    ...
    if (isAbortError) {
      ...
      // 检测上下文超出错误，自动触发压缩重试（流内错误场景，Fusion 模式不触发）
      if (!_isCompressedRetry && isContextOverflowError(errMsg) && historyMessages.length > 0 && provider !== "fusion") {
      ...
      } else if (provider === "fusion" && hasFusionRuntimeProgress) {
        preserveFusionErrorState = true;
      } else {
        await rollbackPendingTurn(convIdForSync);
      }
