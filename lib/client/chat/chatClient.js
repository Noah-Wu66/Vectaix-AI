...
export async function runChat({
  ...
  try {
    ...
    let hasInterruptedStream = false;
    ...
    const markInterruptedStream = (message) => {
      const safeMessage = typeof message === "string" && message.trim()
        ? message.trim()
        : "本次回答被模型服务中断，以下内容可能不完整";
      const last = getLastTimelineStep();
      if (last?.kind === "writer" && last?.status === "error" && last?.message === safeMessage) {
        return;
      }
      appendTimelineStep({
        kind: "writer",
    ...
    if (streamErrorMessage && (fullText.trim() !== "" || hasCouncilProgress)) {
      ...
      thinkingEnded = true;
      closeStreamingThoughtSteps();
      markInterruptedStream(
        `本次回答被模型服务中断，以下内容可能不完整：${streamErrorMessage}`,
      );
      flushStreamingMessage();
    }
    ...
    if (!signal?.aborted && !hasInterruptedStream && (sawDone || fullText.length > 0)) {
      await playCompletionSound();
    }

    // 流式结束后做一次"最终对齐"：移动端偶发断流/缓冲时，避免必须刷新才能看到完整内容
    if (!signal?.aborted && !hasInterruptedStream && sawDone && convIdForSync) {
      (async () => {
        try {
