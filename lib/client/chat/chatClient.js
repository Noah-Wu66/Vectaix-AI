...
import {
  ...
  buildAgentMessageSnapshot,
  createAgentEventState,
} from "@/lib/client/agent/eventReducer";
...
function resolveDirectChatEndpoint(modelId) {
  ...
  if (provider === "claude" || provider === "xiaomi" || provider === "minimax") return "/api/anthropic";
...
export async function runChat({
  ...
  const usesAgentRuntime = !isCouncilModel(model) && normalizedRuntimeMode === CHAT_RUNTIME_MODE_AGENT;
  const modelProvider = getModelConfig(model)?.provider || "";
  const provider = isCouncilModel(model) ? "council" : (usesAgentRuntime ? "agent" : modelProvider);
  ...
  const apiEndpoint = isCouncilModel(model)
    ? "/api/council"
    : (usesAgentRuntime ? "/api/agent" : resolveDirectChatEndpoint(model));
  ...
  try {
    ...
    const agentEventState = provider === "agent"
      ? createAgentEventState({
          inheritedMessage: {
            thought: _inheritedThought || "",
            thinkingTimeline: _inheritedTimeline || [],
            citations: _inheritedCitations || null,
            tools: _inheritedTools || null,
            artifacts: _inheritedArtifacts || null,
          },
        })
      : null;
    let done = false;
    ...
    const syncAgentDerivedFields = () => {
      if (!agentEventState) return;
      const snapshot = buildAgentMessageSnapshot({}, agentEventState);
      fullText = agentEventState.content;
      displayedText = agentEventState.content;
      fullThought = agentEventState.thought;
      citations = agentEventState.citations;
      thinkingTimeline = agentEventState.thinkingTimeline;
      isSearching = snapshot.isSearching;
      searchQuery = agentEventState.searchQuery;
      searchResults = agentEventState.searchResults;
      searchError = agentEventState.searchError;
      thinkingEnded = agentEventState.thinkingEnded || agentEventState.runtimeEnded;
      if (agentEventState.runtimeError) {
        streamErrorMessage = agentEventState.runtimeError;
      }
    };
    ...
    const flushStreamingMessage = () => {
      ...
      setMessages((prev) => {
        ...
        const base = prev[idx];
        const nowHasContent = agentEventState
          ? agentEventState.hasReceivedContent
          : (
        ...
        const nextMsg = agentEventState
          ? {
              ...buildAgentMessageSnapshot(base, agentEventState),
              councilExperts,
              councilExpertStates,
              councilSummaryState,
    ...
    const applyEventPayload = (payload) => {
      ...
      try {
        ...
        if (agentEventState) {
          applyAgentRuntimeEvent(agentEventState, data);
          syncAgentDerivedFields();
          if (shouldDelayConversationActivation && agentEventState.content.trim()) {
            ensureConversationActivated();
          }
          if (data.type === "agent_runtime_end") {
            sawDone = true;
          }
          scheduleFlush();
          return;
