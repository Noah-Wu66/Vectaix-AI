...
export async function GET(req) {
  ...
  // Rate limit: 200 image downloads per hour per user
  const clientIP = getClientIP(req);
  const rlKey = `img-download:${auth.userId}:${clientIP}`;
  const { success: rlOk, resetTime } = await rateLimit(rlKey, { limit: 200, windowMs: 60 * 60 * 1000 });
  if (!rlOk) {
