...
Vectaix AI는 Vercel 클라우드 네이티브 배포를 위해 설계된 **오픈소스 AI 워크스페이스**입니다. 세계 최고의 AI 모델들을 하나의 통합 인터페이스에 모아 — 공식 API 지원, 지능형 라우팅, 고유한 멀티전문가 협업 시스템을 제공합니다.
...
### 🧠 Council 워크플로우
고유한 멀티전문가 협업 모드: GPT-5.4, Claude Opus 4.6, Gemini 2.5 Pro가 병렬 전문가로 답변하고, Seed 2.0 Pro가 최종 응답을 종합.

### 🔀 스마트 모델 라우팅
사용자 수준의 공식 API와 OpenRouter 간 라우트 전환으로 유연성과 비용 관리를 실현.
...
| 모델 | 제공업체 | 라우팅 | 특징 |
|:------|:---------|:-------|:-----|
| **GPT-5.4** | OpenAI | 공식 / OpenRouter | 범용 지능, 코딩, 분석 |
| **Claude Opus 4.6** | Anthropic | 공식 / OpenRouter | 깊은 추론, 글쓰기, 안전성 |
| **Gemini 2.5 Pro** | Google | 공식 / OpenRouter | 멀티모달, 긴 컨텍스트 |
| **DeepSeek V3.2** | DeepSeek | 공식만 | 추론, 수학, 코드 |
| **Seed 2.0 Pro** | ByteDance | 공식만 | 중국어, 요약 |
| **MiMo V2 Flash** | Xiaomi | OpenRouter만 | 빠른 추론 |
| **MiniMax M2.5** | MiniMax | OpenRouter만 | 다국어 생성 |
...
| `OPENROUTER_API_KEY` | ✅ | OpenRouter 게이트웨이 |
